...
function admin(req, res) {
    ...
    //general
    //due to the checking for generals done below, all updates are never going to run because it treats this as though it's a new gen
    general.findOne({ $or: [{ email: email }, { username: code }] }, (err, ret_g) => {
        if (err) {
            throw err;
        }
        else if (ret_g) {
            setNotif("Email or code in use. Select another", false);
            res.redirect('/admin');
        }
        else {
            ...
            legis.findOne({ email: email }, (err, f_leg) => {
                if (err) {
                    ...
                    setNotif('Error: Email address already in use. Choose another', false);
                    res.redirect('/admin');
                }
                else {
                                                ...
                                                if (err) {
                                                    ...
                                                    newLeg.save((err) => {
                                                        if (err) {
                                                            ...
                                                            setNotif("State, district and legislator added successfully", true);
                                                            res.redirect('/admin');
                                                        }
                                                    });
                                                }
                                        ...
                                        if (err) {
                                            ...
                                            dists.findOneAndUpdate({ code: code }, newDist, (err) => {
                                                if (err) {
                                                    ...
                                                    setNotif('Updated Legislator and District to reflect new name', true);
                                                    res.redirect('/admin');
                                                }
                                            });
                                        }
                                                        ...
                                                        if (err) {
                                                            ...
                                                            states.findOneAndUpdate({ state_code: s_code }, { sen_dists: ret_sen_dists }, (err) => {
                                                                if (err) {
                                                                    ...
                                                                    setNotif("District and legislator added successfully", true);
                                                                    res.redirect('/admin');
                                                                }
                                                            });
                                                        }
    ...
    function setNotif(notification, isNotif) {
        req.notifications = { notification: notification, notif: isNotif};
    }
}
