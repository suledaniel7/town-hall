...
function signup(req, res) {
    ...
    //validating data
    if (wsp.test(f_name) || wsp.test(username) || wsp.test(email) || wsp.test(password) || wsp.test(gender) || wsp.test(sen_dist) || wsp.test(fed_const) || wsp.test(v_id)) {
        ...
        general.findOne({ $or: [{ email: email }, { username: username }] }, (err, ret_g) => {
            ...
            user.findOne({ v_id: v_id }, (err, t_u) => {
                if (err) {
                    ...
                    res.send(JSON.stringify({ success: false, reason: "A user exists with the entered Voter ID. Please ensure you entered it accurately" }));
                }
                else if (wsp.test(v_id)) {
                    res.send(JSON.stringify({ success: false, reason: "Voter ID is required" }));
                }
                else {
